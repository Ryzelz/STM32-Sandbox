...
int main(void)
{
  ...
  if (periph_uart_init(&huart2) == false)
   ...
   if (periph_i2c_init(&hi2c3) == false)
   {
 	  assert(false);
   }
...
{
    char buffer[100];
    int16_t x = mpu6050_get_x_axis_data();
    int16_t y = mpu6050_get_y_axis_data();
    int16_t z = mpu6050_get_z_axis_data();

    uint16_t len = snprintf(buffer, sizeof(buffer), "X: %d | Y: %d | Z: %d\r\n", x, y, z);
    periph_uart_send_tx_data(buffer, len);
}
/* USER CODE END 4 */
